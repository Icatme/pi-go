...
func resolveModelRef(provider, model, authRoot string) (core.ModelRef, error) {
	...
	if err != nil {
		return core.ModelRef{}, fmt.Errorf("mattermost-chatbot: auth root %q does not contain .pigo: %w", authRoot, err)
	}
	if !info.IsDir() {
		return core.ModelRef{}, fmt.Errorf("mattermost-chatbot: auth root %q has a non-directory .pigo entry", authRoot)
	}
	...
	if pigo.RequiresOAuth(pigo.Provider(provider)) {
		return core.ModelRef{}, fmt.Errorf("mattermost-chatbot: missing OAuth credentials for %s in %s", provider, filepath.Join(supportDir, "auth.json"))
	}
	if apiKeyEnv != "" {
		return core.ModelRef{}, fmt.Errorf("mattermost-chatbot: missing API key %s for %s in environment or %s", apiKeyEnv, provider, filepath.Join(supportDir, ".env"))
	}
	return core.ModelRef{}, fmt.Errorf("mattermost-chatbot: no credential strategy is configured for provider %s", provider)
}
...
func loadAuthFile(path string) (map[string]storedOAuthCredentials, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("mattermost-chatbot: failed to read %s: %w", path, err)
	}

	var auth map[string]storedOAuthCredentials
	if err := json.Unmarshal(body, &auth); err != nil {
		return nil, fmt.Errorf("mattermost-chatbot: failed to parse %s: %w", path, err)
	}
	if auth == nil {
