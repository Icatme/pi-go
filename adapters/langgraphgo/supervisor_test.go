...
func TestCompileSupervisorRoutesToSelectedMember(t *testing.T) {
	...
	runnable, err := registry.CompileSupervisor(SupervisorConfig[supervisorState]{
		Router: agent.AgentDefinition{
			Model: router,
		},
		GetMessages: func(state supervisorState) []agent.Message { return state.Messages },
	...
	result, err := runnable.Invoke(context.Background(), supervisorState{
		Messages: []agent.Message{agent.NewTextMessage(agent.RoleUser, "please research this topic")},
	})
	if err != nil {
...
func TestCompileSupervisorForStateUsesSelector(t *testing.T) {
	...
	initial := supervisorState{
		Messages: []agent.Message{agent.NewTextMessage(agent.RoleUser, "delegate this to the writer")},
		Mode:     "writer-only",
	}
...
func TestCompileSupervisorRouterUsesThreadIDAsSessionID(t *testing.T) {
	...
	result, err := runnable.InvokeWithConfig(context.Background(), supervisorState{
		Messages: []agent.Message{agent.NewTextMessage(agent.RoleUser, "nothing to do")},
...
func compileSupervisorWorker(t *testing.T, name string) *graph.StateRunnable[supervisorState] {
	...
	workflow.AddNode(name, "worker "+name, func(_ context.Context, state supervisorState) (supervisorState, error) {
		...
		state.Messages = append(state.Messages, agent.Message{
			...
			Parts:     []agent.Part{{Type: agent.PartTypeText, Text: "worker:" + name}},
...
func (m *supervisorRouterModel) Stream(_ context.Context, request agent.ModelRequest) (agent.AssistantStream, error) {
	...
	return newStaticAssistantStream(agent.Message{
		Role:       agent.RoleAssistant,
		ToolCalls:  []agent.ToolCall{{Name: "route", Arguments: args}},
		Timestamp:  time.Now().UTC(),
		StopReason: agent.StopReasonToolUse,
	}, nil), nil
}

func routeOptions(request agent.ModelRequest) []string {
...
func lastTextMessage(messages []agent.Message) string {
