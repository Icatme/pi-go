...
type SupervisorConfig[S any] struct {
	...
	GetMessages  func(S) []agent.Message
...
func routeSupervisorNext[S any](ctx context.Context, definition agent.AgentDefinition, messages []agent.Message, members []RegisteredSupervisorMember[S], routerPrompt string, threadID string) (string, error) {
	snapshot := normalizeSnapshotSessionID(agent.AgentSnapshot{}, threadID)
	...
	stream, err := model.Stream(ctx, agent.ModelRequest{
		Model:           modelRef,
		SystemPrompt:    buildSupervisorPrompt(definition.SystemPrompt, routerPrompt, members),
		Messages:        converted,
		Tools:           []agent.ToolDefinition{buildSupervisorRouteTool(members)},
...
func buildSupervisorRouteTool[S any](members []RegisteredSupervisorMember[S]) agent.ToolDefinition {
...
func parseSupervisorRoute(message agent.Message, options []string) (string, error) {
