...
func TestSessionStateNodeConsumesSteeringAfterAssistant(t *testing.T) {
	...
	node := NewSessionStateNode(nil, agent.AgentDefinition{
		Model: staticModel{
			streamFn: func(_ context.Context, request agent.ModelRequest) (agent.AssistantStream, error) {
				...
				return newStaticAssistantStream(agent.Message{
					...
					Parts:      []agent.Part{{Type: agent.PartTypeText, Text: "handled"}},
	...
	next, err := node(context.Background(), SessionState{
		Snapshot: agent.AgentSnapshot{
			Messages: []agent.Message{
				{
					Role:       agent.RoleAssistant,
					Parts:      []agent.Part{{Type: agent.PartTypeText, Text: "done"}},
					Timestamp:  time.Now().UTC(),
					StopReason: agent.StopReasonStop,
				},
			},
		...
		Steering: []agent.Message{
			agent.NewTextMessage(agent.RoleUser, "nudge"),
		},
...
func TestCheckpointableSessionStateGraphAutoResumeWithPrompt(t *testing.T) {
	...
	first, err := runnable.InvokeWithConfig(ctx,
		PromptUpdate(agent.NewTextMessage(agent.RoleUser, "first")),
	...
	second, err := runnable.InvokeWithConfig(ctx,
		PromptUpdate(agent.NewTextMessage(agent.RoleUser, "second")),
		graph.WithThreadID(threadID),
	)
	if err != nil {
...
func TestCheckpointableSessionStateGraphUsesThreadIDAsSessionID(t *testing.T) {
	...
	threadID := t.Name()
	g := NewCheckpointableSessionStateGraph(nil, agent.AgentDefinition{
		Model: staticModel{
			streamFn: func(_ context.Context, request agent.ModelRequest) (agent.AssistantStream, error) {
				...
				return newStaticAssistantStream(agent.Message{
					Role:       agent.RoleAssistant,
					Parts:      []agent.Part{{Type: agent.PartTypeText, Text: "echo: " + lastUserText(request.Messages)}},
					Timestamp:  time.Now().UTC(),
					StopReason: agent.StopReasonStop,
				}, nil), nil
			},
	...
	result, err := runnable.InvokeWithConfig(context.Background(),
		PromptUpdate(agent.NewTextMessage(agent.RoleUser, "hello")),
		graph.WithThreadID(threadID),
	)
	if err != nil {
...
func TestCheckpointableSessionStateGraphInterruptAfterAndResume(t *testing.T) {
	...
	result, err := runnable.InvokeWithConfig(ctx,
		PromptUpdate(agent.NewTextMessage(agent.RoleUser, "phase one")),
		config,
	)
	if err == nil {
	...
	resumeConfig, err := UpdateSessionState(ctx, runnable, graph.WithThreadID(threadID),
		PromptUpdate(agent.NewTextMessage(agent.RoleUser, "phase two")),
	)
	if err != nil {
		t.Fatalf("update session state failed: %v", err)
	}
