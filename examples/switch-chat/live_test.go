...
func TestLiveSmokeChatAndReflection(t *testing.T) {
	if os.Getenv("PI_GO_AGENT_EXAMPLE_LIVE") != "1" {
		t.Skip("set PI_GO_AGENT_EXAMPLE_LIVE=1 to run live switch-chat example tests")
	}

	authRoot := os.Getenv("PI_GO_AGENT_EXAMPLE_AUTH_ROOT")
	if authRoot == "" {
		t.Skip("set PI_GO_AGENT_EXAMPLE_AUTH_ROOT to a directory that contains .pigo credentials")
	}

	provider := os.Getenv("PI_GO_AGENT_EXAMPLE_PROVIDER")
	if provider == "" {
